prediction3 <- function(input, dfquad, dftri, dfbi, dfword, num){
  ...
  # }
  # dftri1 <- dplyr::filter(dftri, start == words2)
  # dftri1$prob <- 0.65*dftri1$prob
  # dftri1 <- dftri1[with(dftri1, order(-prob)), ]
  # 
  # dfbi1 <- dplyr::filter(dfbi, start == words1)
  # dfbi1$prob <- 0.3*dfbi1$prob
  # dfbi1 <- dfbi1[with(dfbi1, order(-prob)), ]
  # 
  # dfword$prob <- 0.05*dfword$prob
  # dfword <- dfword[with(dfword, order(-prob)), ]
  # 
  # results <- rbind(dfquad1, dftri1, dfbi1)
  # results <- results[,c(2,3)]
  # results <- rbind(results, dfword[1:5,])
  # results <- results %>% group_by(end) %>% summarise(prob = sum(prob))
  # #results <- results[!duplicated(results$end),]
  # results <- results[with(results, order(-prob)), ]
  # if(num == 1){
  #   return(results$end[1])
...
# testPred3 <- function(sentence, df4, df3, df2, df1, num){
...
#   
#   
#   pred <- prediction3(sentence, df4, df3, df2, df1, num)
#   
#   if(endWord == pred){
#     count <- count + 1
#     return(count)
...
# 
# for(i in sampleData){
#   correct <- testPred3(i, df4, df3, df2, df1, 1)
#   number <- number + 1
#   print(number)
#   count <- count + correct
# }
